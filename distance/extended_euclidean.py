...
def createDicts(event_log):
    ...
    for id in event_log.case_id.unique() :
        ...
        for obj in objects:
            action_vector = []
            for act in actions:
                action_vector.append(len(trace[(trace['business_object']==obj) & (trace['action']==act)]))
            dict_trace[obj]=action_vector
        dicts.append(dict_trace)
        i+=1

    return dicts, objects, actions
...
def computeMean(traces, objects, actions, no):
    ...
    for obj in objects:
        ...
        for n in range(0, len(actions)):
            ...
            if len(traces) != 0:
                for trace in traces:
                    sum += trace[obj][n]
                action_vector.append(sum/len(traces))
            else:
                action_vector.append(0)
        mean[obj]=action_vector
    return mean